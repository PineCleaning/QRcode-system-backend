 ...
 * thumbnails), with cards that need attention tinted warm and flagged
 * items badged. Originally styled after a reference inspection-app
...
export class InspectionReportService {
  ...
  private drawSummaryBox(doc: PDFKit.PDFDocument, inspection: ReportInspection) {
    ...
    if (flaggedCount > 0) {
      doc.fillColor(CORAL_DARK).fontSize(10).font('Helvetica-Bold').text(`${flaggedCount} item${flaggedCount === 1 ? '' : 's'} flagged for follow-up`, PAGE_LEFT, doc.y);
      doc.moveDown(0.3);
    }
    doc.moveDown(0.5);
  }
  ...
  private drawItem(doc: PDFKit.PDFDocument, item: ReportItem, thumbnails: Map<string, Buffer>) {
    ...
    // The name shares its row with the flagged pill (left) and the rating
    // pill + percentage (right) - cap its width so a long name is cut
    // with an ellipsis instead of wrapping into the bar underneath.
    const flaggedPillWidth = item.flagged ? this.pillWidth(doc, 'Flagged') + 8 : 0;
    doc.fontSize(13).font('Helvetica-Bold');
    const nameText = fitText(doc, item.spaceName, 250 - flaggedPillWidth);
    ...
    if (item.flagged) {
      ...
      this.drawPill(doc, 'Flagged', innerLeft + nameWidth + flaggedPillWidth, rowTop, RATING_STYLES.VERY_POOR);
    }
  ...
  private pillWidth(doc: PDFKit.PDFDocument, label: string): number {
    doc.fontSize(9).font('Helvetica-Bold');
    return doc.widthOfString(label) + 18;
  }
