...
- **CRM:** ClickUp OAuth + direct API integration, plus n8n (parallel webhook path, owned by client's own developer)
- **Deployment:** Railway
...
`DATABASE_URL`, `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `SUPABASE_ANON_KEY`, `CLICKUP_CLIENT_ID`, `CLICKUP_CLIENT_SECRET`, `CLICKUP_REDIRECT_URI`, `CLICKUP_WORKSPACE_ID`, `CLICKUP_LIST_ID`, `CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET`, `N8N_WEBHOOK_URL`, `BASE_DOMAIN`.
...
- **Dual ClickUp delivery must be independent** — the direct ClickUp API call and the n8n webhook fire separately (separate try/catch or async tasks). One path failing/slow must never block or hide the other. Log both paths independently (`clickup_delivery_status`, `n8n_delivery_status`).
