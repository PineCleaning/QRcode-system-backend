...
- **Deliberately not included, flagged as separate follow-ups, not silently bundled in:** rate limiting (discovery doc Section 5.1 wants it on login/upload-signing/QR-resolution/feedback-submission — needs `@nestjs/throttler`, a new dependency) and restricting CORS to specific origins (currently `app.enableCors()` is wide open — can't be meaningfully restricted until the production domain from Open Decision #7 is locked in). Both are real gaps from the earlier discovery-doc audit, just out of scope for "error handling + validation" specifically.
