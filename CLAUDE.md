...
Audited first, then fixed only real gaps — per-service error handling (404s via explicit `findOne` before update/delete, 409 via Prisma `P2002`/`P2003` mapping, non-blocking ClickUp sync) was already solid throughout, built in as each feature went in rather than deferred to this pass.

- **`src/prisma/prisma.service.ts`** — now throws a clear `DATABASE_URL is required but was not set` error at construction time if unset, instead of `new PrismaPg({ connectionString: undefined })` failing later with a confusing low-level `pg` error. Matches the `config.getOrThrow(...)` fail-fast pattern already used by `CloudinaryService`, `SupabaseService`, and `ClickupConnectionService` for their required config — this was the one service not already following it.
- **`src/common/all-exceptions.filter.ts`** (`AllExceptionsFilter`, registered via `app.useGlobalFilters()` in `main.ts`) — passes every `HttpException` through **completely untouched** (verified via curl: 401/400/404 response bodies are byte-identical to before), so no existing error behavior changed anywhere in the app. Its only job is the previously-uncaught case: an unexpected non-`HttpException` error (a real bug, an unhandled Prisma error, etc.) now gets logged server-side with full stack trace and request context via `Logger`, while the client only ever sees a safe generic `{ statusCode: 500, message: "Something went wrong. Please try again." }` — guarantees internals never leak even for a case nobody anticipated.
- **Deliberately not included, flagged as separate follow-ups, not silently bundled in:** rate limiting (see its own section below, since implemented) and restricting CORS to specific origins (currently `app.enableCors()` is wide open — can't be meaningfully restricted until the production domain from Open Decision #7 is locked in). CORS remains a real gap from the earlier discovery-doc audit, out of scope here.

## Rate Limiting (implemented 2026-07-28)

`@nestjs/throttler` (v6, ttl in ms), applied per-route via `@UseGuards(ThrottlerGuard)` + `@Throttle(...)` — **not** registered as a global `APP_GUARD`. `ThrottlerModule.forRoot(...)` in `app.module.ts` exists only to provide the storage/reflector for DI; its "default" throttler entry is never actually reached since every guarded route below specifies its own limit.

- **Login has no backend rate limit** — it isn't a backend route at all; Supabase Auth handles it directly.
- **`POST /feedback/:slug`** — 5 requests / 10 min, keyed by **IP + slug** (not IP alone) via a per-route `getTracker`. A real customer submits once, maybe retries once or twice on a bad connection; keying by slug too means multiple customers sharing one public IP (job-site WiFi/NAT) can't lock each other out over different QR scans.
- **`POST /uploads/cloudinary-signature`** — 10 requests / 10 min per IP. Max 5 attachments per submission means legitimate use is 1–3 calls per session; kept tighter than feedback submission itself since each signature is effectively "permission to push a file to Cloudinary" — an abuse/cost vector even before a submission happens.
- **`GET /public/:slug`** — 30 requests / min per IP. Fires once per scan, but phones on flaky connections retry a form load a few times; no sensitive data returned, so the risk is load/scraping across slugs, not leakage.
- **Admin routes are untouched** — already behind `SupabaseAuthGuard`, no second layer added.
- **`main.ts` sets `app.set('trust proxy', 1)`** — required once deployed behind Railway's reverse proxy, otherwise every request's `req.ip` resolves to the proxy itself and IP-based limiting either blocks everyone together or does nothing. Easy to forget since it has zero effect locally (no proxy in front of `localhost`) — only shows up as a bug in production.

Live-verified 2026-07-28: fired 32 rapid requests at `/public/:slug` — requests 1–30 passed through, 31–32 returned `429`. Fired 6 requests at `/feedback/throttle-test-a` — the 6th returned `429`, then immediately confirmed a request to `/feedback/throttle-test-b` from the same IP was unaffected (`404`, not `429`), proving the IP+slug key genuinely isolates different slugs rather than rate-limiting by IP alone.

## CSV Bulk Import (Day 2 Hr 4 — implemented 2026-07-28)

`src/csv-import/` — `POST /clients/bulk-upload`, admin-guarded, multipart file upload (`FileInterceptor('file')` from `@nestjs/platform-express`, backed by `multer`, already a transitive dependency - no new native binary). New isolated module rather than adding to `ClientsController` (`@Controller('clients')` on both, no path collision since none of `ClientsController`'s routes are `POST /clients/:something`) - keeps this substantially larger batch-processing logic out of the simple single-resource CRUD controller.

- **Open Decision #4 (exact CSV column format) is still genuinely unresolved** - the client's real spreadsheet was never provided. Built against a sensible default template instead (confirmed with the user 2026-07-28, same self-activating philosophy as ClickUp/Cloudinary all session): `Client Name, Client Code (optional), Contact Email, Contact Phone, Site Name, Address`. Header matching goes through a `HEADER_ALIASES` lookup (`csv-import.service.ts`), tolerant of common variations (e.g. "Business Name"/"Client Name", "Location"/"Site Name") rather than one exact spelling. **When the real format arrives, adjusting this one alias map is the expected, contained change** - nothing else in the file should need touching for that.
- **One CSV row = one client+site pair.** Client is matched by `clientCode` when the row provides one (create-or-update - update only overwrites fields the row actually provided); when blank, a code is auto-generated (`slugify(name) + '-' + random 4-digit suffix`, retried up to 5x on a uniqueness collision) - mirrors the frontend's own `ClientForm.tsx` auto-gen behavior (Section further down this doc), reimplemented server-side since there's no interactive "Regenerate" button in a bulk/automated context.
- **Sites have no CSV-provided key** (`site_code`/`slug` are always system-generated, never admin-provided - same rule as the single-site-create API and the portal UI). Matched instead by `clientId` + `siteName` (case-insensitive) so re-running the same CSV updates rather than duplicates - `site_code`/`slug` generation itself duplicates `SitesService`'s generate-try-retry-on-collision shape rather than reusing it directly, since that method's DTO/404 signature is tied to a single-client HTTP request, not this per-row batch loop.
- **ClickUp sync per created/updated client is non-blocking**, same pattern as `ClientsService.syncToClickup` (duplicated rather than calling the private method directly, to keep `ClientsController`'s working CRUD flow completely untouched) - a ClickUp outage never fails a row, just logs a warning.
- **Per-row, not whole-batch, error handling** - every row gets its own `csv_import_rows` entry (`SUCCESS`/`ERROR` + `errorMessage`), and one bad row never aborts the rest of the file. Response returns the full batch summary + every row's result synchronously (no separate polling/history endpoint built - the roadmap's "clear row-by-row results" requirement is satisfied by this one response).
- **Max 1000 rows per file**, matching the discovery doc's stated CSV import cap (Section 5.2) - enforced before any DB writes happen.

Verified: CSV parsing + header-alias mapping tested directly against a real sample file (multi-row same-client-code grouping, blank-client-code auto-gen path, both parsed correctly). Route mapped and `401`s without a token, same as every other admin route; `git diff` confirmed zero touches to `ClientsController`/`ClientsService`/`SitesService`. Full authenticated upload-through-Supabase flow needs a real browser session to verify (same limitation as every other feature this session lacking test credentials for curl).
